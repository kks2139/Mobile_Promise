...
*/
        ...
        list.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            ...
            public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
                ...
                for(int i=0;i<index.size();i++){
                    Toast.makeText(MainActivity.this,"음... = "+index.get(i)+" "+index2.get(i),Toast.LENGTH_SHORT).show();
    ...
    public boolean onKeyDown(int keyCode, KeyEvent event)// back 버튼으로 종료할시, 바로 끄지않고 종료확인 메시지를 한번 더 띄워준다다
    {
        ...
        {
            new AlertDialog.Builder(this).setIcon(android.R.drawable.ic_dialog_alert).setTitle("종료?").setMessage("진짜로 그만?").setPositiveButton("예", new DialogInterface.OnClickListener()
            {
                @Override
                public void onClick( DialogInterface dialog, int which )
                {
